...
def fetch_top_movie_urls
  ...
  links = []
  html_doc.search('a.ipc-title-link-wrapper').first(5).each do |tag|
    links << "https://www.imdb.com" + tag.attributes['href'].value.gsub(/\?.*$/,'')
  end
...
def scrape_movie(url)
  ...
  # TODO: return movie info hash
  {
    ...
    storyline: html_doc.search('.sc-7193fc79-0.gdjunN').text
  }
end
